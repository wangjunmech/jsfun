...
//**添加元素,需要jquery并放入文件中，引用无效
function appendText(target,tag,text){
var txt="<"+tag+">"+text+"</"+tag+">";    // 以 HTML 创建新元素
target.append(txt);         // 追加新元素
}

//**取得某一范围的随机数
function randnum(min,max){
  var gap=max-min;
  var randgap = gap*Math.random();
  var resnum = min+randgap;
  return(Math.round(resnum));
  // return (Math.random());
}
